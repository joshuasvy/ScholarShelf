...
export default function Categories() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-4 justify-items-center lg:px-0">
      {topics.map((topic) => (
        <button
          onClick={() => navigate(`/catalog/${encodeURIComponent(topic.name)}`)}
          className="w-50 h-12 md:54 lg:w-58 lg:h-14 flex flex-row justify-center items-center gap-3 rounded-md shadow-md cursor-pointer hover:bg-secondary transition-colors duration-100"
        >
          <img src={topic.icon} alt={topic.icon} className="w-5 md:w-6" />
          <span className="font-inter font-semibold text-md md:text-lg">
            {topic.name}
          </span>
        </button>
      ))}
